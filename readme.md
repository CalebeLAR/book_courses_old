...
`dia03:` Um baralho pythônico	1.1 De que se trata estelivro?	Of JavaScript Runtimes and Building (Mostly) Servers	1.1 Lógica de   
`dia04:` Como os métodos especiais são usados	Quais tipos dedados?	First Baby Steps with Node: Installation	1.1.1 Compreender o que é   
`dia05:` Emulando tipos numéricos	1. 2 Por que Python para análise dedados?	More Useful: Executing JavaScript Source Files	1.1.2 Realizar deduções na construção do   
### semana02  
`dia06:` Representação em string	Python como aglutinador	Node’s Partner in Crime:NPM	1.1.3 Enumerar as etapas a serem   
`dia07:` Operadores aritméticos	Resolvendo o problema de "duas linguagens	A Few More NPM Commands	1.1.4 Analisar outras possibilidades de   
`dia08:` Valor booleano de um tipo definido pelo usurio	Por que não Python?	Initializing a New NPM/Node Project	1.1.5 Ensinar ao computador uma   
`dia09:` Visão geral dos métodos especiais	1. 3 Bibliotecas Python essenciais	Adding Dependencies	1.1.6 Pensar em todos os   
`dia10:` Por que len não é um método?	NumPy	A Quick Aside: Semantic Versioning	1.2 Entrada, processamento e   
### semana03  
`dia11:` Resumo do captulo	pandas	Fisher Price’s “My First Node Web Server	1.3   
`dia12:` Leituras complementares	matplotlib	Bonus Example	1.4 Editores de código   
...
`dia14:` Visão geral das sequências embutidas	SciPy	NPM: More on packagejson	1.6 Variáveis e   
`dia15:` List comprehensions e expressões geradoras	scikitlearn	NPM: Other Commands	1.7 Entrada de dados com prompt  
### semana04  
`dia16:` List comprehensions e legibilidade	statsmodels	Auditing Package Security	1.8   
`dia17:` Comparação entre listcomps e map/filter	1.4 Instalação e configurao	Deduplication and Pruning	1.9 Tipos de dados e conversões de   
`dia18:` Produtos cartesianos	Windows	Finding/Searching for Packages sans Browser	1.10 Exemplos de entrada, processamento e   
`dia19:` Expressões geradoras	Apple (OS X.MacOS)	About the Authorxv	1.11   
`dia20:` Tuplas não são apenas listas imutveis	GNULinux	About the Technical Reviewerxvii	1.12 Considerações finais do   
### semana05  
`dia21:` Tuplas como registros	Instalando ou atualizando pacotesPython	Acknowledgmentsxix	capítulo 2 Integração com   
`dia22:` Desempacotamento de tuplas	Python 2 e Python	Introductionxxi	2.1 Estrutura básica de um documento   
`dia23:` Desempacotamento de tuplas aninhadas	Ambientes de desenvolvimento integrado (IDEs) e editoresde	Updating Packages	2.2 Cabeçalhos, parágrafos e campos de   
`dia24:` Tuplas nomeadas	texto	Publishing/Unpublishing Packages	2.3 Criação do programa   
...
### semana06  
`dia26:` Fatiamento	1. 6 Navegando pelolivro	File System (fs)	2.5 Introdução a eventos e   
`dia27:` Por que as fatias e os intervalos excluem o últimoitem	Exemplos decdigo	HTTP and HTTPS (http and https)	2.6 Propriedades innerText, innerHTML e   
`dia28:` Objetosslice	Dados para os exemplos	OS (os)	2.7 Método preventDefault  
`dia29:` Fatiamento multidimensional e reticências	Convenções de importao	Path (path)	2.8 Operadores aritméticos e funções   
`dia30:` Atribuição de valores a fatias	Jargão	Process	2.9 Exemplos de programas JavaScript integrados com   
### semana07  
`dia31:` Usando + e * com sequências	Capítulo 2. Básico da linguagem Python, IPython e notebooks Jupyter	Query Strings (querystring)	2.10   
`dia32:` Criando listas de listas	2 .1 InterpretadorPython	URL (url)	2.11 Considerações finais do   
`dia33:` Atribuições combinadas e sequências	2. 2 Básico sobre o IPython	Utilities (util)	capítulo 3 Construção de algoritmos com Node.  
`dia34:` O enigma da atribuio	Executando o shell IPython	The Rest of the Cast	3.1 Instalação do Node.  
`dia35:` list.sort e a função embutida sorted	Executando o notebook Jupyter	Chapter 3: Client-Side Adventures: React	3.2 Adição de pacote para entrada de   
### semana08  
`dia36:` Administrando sequências ordenadas com bisect	Preenchimento automático com tabulaco	A Brief History of React	3.3 Criação e execução de programas com o Node.  
`dia37:` Pesquisando com bisect	Introspeco	Yeah, Okay, History Nerd, That’s All Great, but What IS React?	3.4 Exemplos de algoritmos com Node.  
`dia38:` Inserção com bisect.insort	Comando	The Real Star of the Show: Components	3.5 Considerações finais do   
`dia39:` Quando uma lista não é a resposta	/orun	Components Need Info: Props	capítulo 4   
`dia40:` Arrays	Executando código da área de transferncia	Components (Sometimes) Need Memory: State	4.1 If...   
### semana09  
`dia41:` Memory Views	Atalhos de teclado no terminal	Making Them Look Good: Style	4.2 Operadores   
`dia42:` NumPy e SciPy	Sobre os comandos mgicos	In the End, Why React?	4.3 Operadores   
`dia43:` Deques e outras filas	Integração com a matplotlib	Chapter 4: A Few More Words: Advanced React	4.4 Operador   
`dia44:` Resumo do capítulo	2. 3 Básico da linguagemPython	A Better Way to Write React Code: JSX	4.5 Switch...   
`dia45:` Leituras complementares	Semântica da linguagem	Yeah, Okay, So What Does It LOOK LIKE?	4.6 Exemplos com HTML e   
### semana10  
`dia46:` Capítulo 3 ■ Dicionários e conjuntos	Tipos escalares	A Slight Detour into Babel Land	4.7 Exemplos com Node.  
`dia47:` Tipos genéricos de mapeamento	Controle defluxo	Compile JSX	4.8   
`dia48:` diet comprehensions	Capítulo 3 • Estruturas de dados embutidas, funções e arquivos	And Now, Put It All Together	4.9 Considerações finais do   
`dia49:` Visão geral dos métodos comuns a mapeamentos	3 .1 Estruturas de dados e sequncias	Table of ConTenTs	capítulo 5   
`dia50:` Tratando chaves ausentes com setdefault	Tupla	Whither Props?	5.1 Repetição com variável de controle: laços   
### semana11  
`dia51:` Mapeamentos com consulta de chave flexvel	Lista	Default Props	5.2 Repetição com teste no início: laços   
`dia52:` defaultdict: outra abordagem para chaves ausentes	Funcões embutidas para sequências	Typing Props	5.3 Repetição com teste no final: laços do..   
`dia53:` Método__ missing	List. set e dict comprehensions	Component Lifecycle	5.4 Interrupções nos laços (break e continue  
`dia54:` Variações dediet	3.2 Funces	Chapter 5: Building a Strong Foundation: TypeScript	5.5 Contadores e   
...
### semana12  
`dia56:` Mapeamentos imutveis	Devolvendo diversos valores	Jumping into the Deep End	5.7 Exemplos de Algoritmos de Repetição com Node.  
`dia57:` Teoria dos conjuntos	Funções são objetos	Beyond the Playground	5.8   
`dia58:` Literais deset	Funcões anônimas (lambdas)	Configuring TypeScript Compilation	5.9 Considerações finais do   
`dia59:` Set comprehensions	Currying: aplicação parcial dos argumentos	The Nitty Gritty: Types	capítulo 6   
`dia60:` Operações de conjuntos	Geradores	String	6.1 Inclusão e exclusão de   
### semana13  
`dia61:` Por dentro de dieteset	Erros e tratamento de exceco	Number	6.2 Tamanho do vetor e exibição dos   
`dia62:` Um experimento para testar o desempenho	3. 3 Arquivos e o sistema operacional	Boolean	6.3 For..of e forEach  
`dia63:` Tabelas hash em dicionrios	Bytes e Unicode com arquivos	Any	6.4 Localizar   
`dia64:` Consequências práticas de como os'dickertos'funcionam	3.4 Concluso	Arrays	6.5 Vetores de   
`dia65:` Leituras complementares	Capítulo 4. Básico sobre o NumPy: arrays e processamento vetorizado	Tuples	6.6 Desestruturação e operador Rest/  
### semana14  
`dia66:` Capítulo 4 ■ Texto versus bytes	4.1 O ndarray do NumPy: um objeto array multidimensional	Enums	6.7 Pesquisar e filtrar   
`dia67:` Falhas de caracteres	Criando ndarrays	Function	6.8 Map, Filter e   
`dia68:` O essencial sobre bytes	Tipos de dados para ndarrays	Object	6.9 Classificar os itens do   
`dia69:` Structs e memory views	Aritmética com arraysNumPy	Null, Void, and Undefined	6.10 Manipulação de vetores com Node.  
`dia70:` Codificadores/decodificadores básicos	Indexacão básica e fatiamento	Custom Type Aliases	6.11   
### semana15  
`dia71:` Entendendo os problemas de codificação/decodificao	Indexacão booleana	Union Types	6.12 Considerações finais do   
`dia72:` Lidando com UnicodeEncodeError	Indexacão sofisticada	TypeScript == ES6 Features for “Free	capítulo 7 Strings e   
`dia73:` Lidando com UnicodeDecodeError	Transposição de arrays e troca deeixos	The let and const Keywords	7.1 Percorrer os caracteres de uma   
`dia74:` SyntaxError ao carregar módulos com codificação inesperada	4. 2 Funções universais: funções rápidas de arrays para todosos	Block Scope	7.2 Converter para letras maiúsculas ou   
`dia75:` Como descobrir a codificação de uma sequência de bytes	elementos	Arrow Functions	7.3 Cópia de caracteres e remoção de espaços da   
### semana16  
`dia76:` BOM: um gremlin útil	4. 3 Programação orientada aarrays	Template Literals	7.4 Localizar um ou mais caracteres na   
`dia77:` Lidando com arquivos-texto	Expressando uma lógica condicional como operações dearray	Default Parameters	7.5 Dividir a string em elementos de   
...
`dia79:` Normalizando Unicode para comparações mais seguras	Métodos para arrays booleanos	Spread and Rest (and as an Added Bonus: Optional Arguments)	7.7 Substituição de   
`dia80:` Case folding	Ordenaco	Destructuring	7.8 Manipulação de   
### semana17  
`dia81:` Funções utilitárias para comparações normalizadas	Unicidade e outras lógicas de conjuntos	Classes	7.9 Strings e datas com Node.  
`dia82:` “Normalização” extrema: removendo acentos	4.4 Entrada e saída de arquivos comarrays	Chapter 6: A Few More Words: Advanced TypeScript	7.10   
`dia83:` Ordenação de texto Unicode	4.5 Álgebralinear	Interfaces	7.11 Considerações finais do   
`dia84:` Ordenação com o Unicode Collation Algorithm	4. 6 Geração de números pseudoaleatrios	Argument/Object Interfaces	capítulo 8 Funções e   
`dia85:` Base de dados Unicode	4. 7 Exemplo: passeios aleatrios	Methods in Interfaces	8.1 Functions e Arrow   
### semana18  
`dia86:` APIs de modo dual para str e bytes	Simulando vários passeios aleatórios de umasvez	Interfaces and Classes	8.2 Funções com passagem de   
`dia87:` str versus bytes em expressões regulares	4.8 Concluso	Extending Interfaces	8.3 Funções com retorno de   
`dia88:` str versus bytes em funções deos	Capítulo 5 • Introdução aopandas	Namespaces and Modules	8.4 Funções com parâmetros   
`dia89:` Resumo do capítulo	5 .1 Introdução às estruturas de dados dopandas	Namespaces	8.5 Funções   
`dia90:` Leituras complementares	Series	Modules	8.6 Eventos   
### semana19  
`dia91:` Capítulo 5 ■ Funções de primeira dasse	DataFrame	Decorators	8.7 Funções com Node.  
`dia92:` Tratando uma função como um objeto	Obietos Index . 5. 2 Funcionalidades essenciais	Decorator Factories	8.8   
`dia93:` Funções de ordem superior	Reindexaco	Third-Party Libraries	8.9 Considerações finais do   
`dia94:` Substitutos modernos para map, filter e reduce	Descartando entradas de umeixo	Debugging TypeScript Apps	capítulo 9 Persistência de dados com   
`dia95:` Funções anônimas	Indexação. seleção e filtragem	Source Maps	9.1 Salvar e recuperar   
### semana20  
`dia96:` As sete variações de objetos invocáveis	Índices inteiros	Chapter 7: Tying It Up in a Bow: Webpack	9.2 Uma “pitada” de   
`dia97:` Tipos invocáveis definidos pelo usurio	Aritmética e alinhamento dedados	What’s a Bundle, and How Do I Make One?	9.3 Remover dados do   
`dia98:` Introspecção de funo	Aplicação de funções e mapeamento	What’s Webpack All About?	9.4 Uso do querySelectorAll  
`dia99:` De parâmetros posicionais a parâmetros exclusivamente nomeados	Ordenacão e classificaco	Dependency Graph	9.5 Manipular listas no   
`dia100:` Obtendo informações sobre parâmetros	Índices de eixos com rótulos duplicados	Entry	9.6   
### semana21  
`dia101:` Anotações de funo	5. 3 Resumindo e calculando estatísticas descritivas	Output	9.7 Considerações finais do   
`dia102:` Pacotes para programação funcional	Correlacão e covarincia	Loaders	capítulo 10 Inserir elementos HTML via   
`dia103:` Módulo operator	Valores únicos. contadores de valores e pertinncia	Plugins	10.1 Inserir e manipular elementos de   
`dia104:` Congelando argumentos com functools.partial	5 .4 Concluso	Modes	10.2 Inserir   
`dia105:` Resumo do captulo	Capítulo 6. Carga de dados, armazenagem e formatos de arquivo	Browser Compatibility	10.3 Manipular tabelas   
### semana22  
`dia106:` Leituras complementares	6.1 Lendo e escrevendo dados em formatotexto	Table of ConTenTs	10.4   
`dia107:` Capítulo 6 ■ Padrões de projeto com funções de primeira classe	Lendo arquivos-texto empartes	Getting Started with Webpack	10.5 Considerações finais do   
...
`dia109:` Strategy clássico	Trabalhando com formatos delimitados	Configuration	11.1 Programa Jockey   
`dia110:` Strategy orientado a funço	DadosJSON	Using Modules	11.2 Programa Reserva de Poltronas em   
### semana23  
`dia111:` Escolhendo a melhor estratégia: abordagem simples	XML e HTML: web scraping	Wither TypeScript?	11.3 Jogo “Descubra a   
`dia112:` Encontrando estratégias em um módulo	6.2 Formatos de dados binrios	Chapter 8: Delivering the Goods: MailBag, the Server	11.3.1 Cadastro de   
`dia113:` Command	Usando o formatoHDFS	What Are We Building?	11.3.2 Listagem de   
`dia114:` Resumo do capítulo	Lendo arquivos do MicrosoftExcel	Basic Requirements	11.3.3 Programação do   
`dia115:` Leituras complementares	6. 3 Interagindo com APisweb	Setting Up the Project	11.4 Considerações finais do   
### semana24  
`dia116:` Capítulo 7 ■ Decoradores de função e closures	6.4 Interagindo com bancos dedados	Source File Rundown	capítulo 12 Criação de um App: Back-end com   
`dia117:` Básico sobre decoradores	6.5 Concluso	Adding Node Modules	12.1   
`dia118:` Quando Python executa os decoradores	Capítulo 7. Limpeza e preparação dosdados	Adding Types	12.2   
`dia119:` Padrão Strategy melhorado com decorador	7 .1 Tratando dados ausentes	A More Convenient Development Environment	12.3 Rotas POST e Formato   
`dia120:` Regras para escopo de variáveis	Filtrando dados ausentes	The Starting Point: maints	12.4   
### semana25  
`dia121:` Closures	Preenchendo dados ausentes	A Quick Detour: Time to Take a REST	12.5 Use o Knex e escolha o banco de   
`dia122:` Declaração nonlocal	7.2 Transformacão dedados	Another Quick Detour: Express, for Fun and Profit	12.6 Criação de tabelas com   
`dia123:` Implementando um decorador simples	Removendo duplicatas	Back to the Code	12.7 Seeds: “semeando” dados   
`dia124:` Funcionamento	Transformando dados usando uma funcãoouum	Gotta Know What We’re Talking to: ServerInfots	12.8 Database Config e express.  
`dia125:` Decoradores da biblioteca-padro	mapeamento	Time to Send the Mail: smtpts	12.9 Async e   
### semana26  
`dia126:` Memoização com functools.lru.cache	Substituindo valores	A Quick Detour: Nodemailer	12.10 Status Code   
`dia127:` Funções genéricas com dispatch simples	Renomeando os índices doseixos	Another Quick Detour: Generics	12.11 Rotas para a realização do   
`dia128:` Decoradores empilhados	Discretização e compartimentalização (binning)	Back to the Code	12.12 Filtros, totalizações e   
`dia129:` Decoradores parametrizados	Detectando e filtrando valores discrepantes	Time to Get the Mail (and Other Stuff): imapts	12.13   
`dia130:` Um decorador de registro parametrizado	Permutação e amostragem aleatria	A Quick Detour: emailjs-imap-client and mailparser	12.14 Considerações finais do   
### semana27  
`dia131:` Decorador clock parametrizado	Calculando variáveis indicadorasdummy	Back to the Code	capítulo 13 Criação de um App: Front-end com   
...
`dia133:` Leituras complementares	Métodos de objetosstring	A Quick Detour: NoSQL	13.2 Organização em   
`dia134:` Capítulo 8 - Referências a objetos, mutabiIidade e reciclagem	Expressões regulares	Another Quick Detour: NeDB	13.3 Criação de um novo projeto   
`dia135:` Variáveis não são caixas	Funções de string vetorizadas nopandas	Table of ConTenTs	13.4 Simplificando o gerenciamento do form com React Hook   
### semana28  
`dia136:` Identidade, igualdade e apelidos	7.4 Concluso	Back to the Code	13.5 Axios para comunicar com o Web   
`dia137:` Escolhendo entre == eis	Capítulo 8. Tratamento de dados: junção, combinação e reformatao	Testing It All	13.6 Criação de Rotas com o React   
`dia138:` A relativa imutabilidade das tuplas	8 .1 Indexação hierrquica	Optional Tooling	13.7 useEffect  
`dia139:` Cópias são rasas por padro	Reorganizando e ordenandonveis	Suggested Exercises	13.8 Filtrando os registros da   
`dia140:` Cópias profundas e rasas de objetos quaisquer	Estatísticas de resumo pornvel	Chapter 9: Delivering the Goods: MailBag, the Client	13.9 Exclusão, alteração e passagem de funções como   
### semana29  
`dia141:` Parâmetros de função como referências	Indexando com as colunas de um DataFrame	What Are We Building?	13.10 Resumo com gráfico ou dashboard do   
...
`dia143:` Programação defensiva com parâmetros mutáveis	Juncões no DataFrame no estilo de bancos dedados	Setting Up the Project	  
...
### semana40  
`dia196:` Herança múltipla e ordem de resolução de métodos	Gerando intervalos dedatas	The Basic Layout: BaseLayouttsx  
...
### semana55  
`dia271:` Tratamento de erros nos exemplos da série flags	dtypes aninhados e campos multidimensionais  
...
### semana60  
`dia296:` O problema do nome de atributo inválido	/otimee  
